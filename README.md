...
默认 `docker-compose.yml` 使用正式版镜像：先构建前端静态资源，再通过 Node 生产服务提供静态页面和服务器端文件管理 API。
...
正式版容器内部监听 `80` 端口，compose 映射到宿主机 `5173` 端口。`docker-compose.yml` 中通过 `VITE_LOCAL_FOLDERS` 配置容器内可访问目录：
...
如果需要 Vite 热重载开发模式，可改用 `Dockerfile.dev`：
