...
import type { FolderStyle } from '@/stores/folderConfig'
...
interface Props {
  ...
  level?: number
  folderStyles?: Map<string, FolderStyle>
}
    ...
    <template v-for="node in nodes" :key="node.path">
      ...
      >
        ...
        <span class="node-name" :title="node.name">
          <!-- 彩色标签（仅一级目录） -->
          <span
            v-if="level === 0 && folderStyles.get(node.path)"
            class="inline-block w-2 h-2 rounded-full mr-1.5"
            :style="{ backgroundColor: folderStyles.get(node.path)?.color }"
          />
          {{ node.name }}
          <!-- 自定义标签 -->
          <span
            v-if="level === 0 && folderStyles.get(node.path)?.label"
            class="text-xs text-muted-foreground ml-1"
          >
            {{ folderStyles.get(node.path)?.label }}
          </span>
        </span>
      </div>
